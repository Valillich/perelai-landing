...
> **Current authority — 2026-09-19 (POS0).** The inventory below describes an older landing and is historical wherever it conflicts with current code or these sources: [Product Marketing Context v11](.agents/product-marketing.md), [messaging rails](.cursor/plans/reference/messaging-and-claims.md), [commercial policy](docs/commercial-policy.md), and [launch-positioning plan](.cursor/plans/positioning/README.md). The launch category is appointments, clients and payment tracking for independent beauty professionals and small studios. The approved offer is a 21-day STUDIO trial without a card, then SOLO USD 19 or STUDIO USD 29 monthly per workspace (1 / up to 5 active performers), subject to BILL/TEAM/legal release evidence. Older finance-only, Founding Beta, no-billing and 19/29/49 tables below are not current rules. Routes are localized under `app/[locale]`, with Home, Pricing, Install and existing niche pages; inspect the current tree instead of restoring the old single-page structure. POS0 changes documentation only. The owner approved six exact RU hero strings on 2026-09-19: «Порядок в записях. Ясность в оплатах.» with the short action body and trial CTA/helper; see the [launch checklist](docs/launch-positioning-checklist.md) §2. Adapt the new UK/EN hero from that source in POS2; other new strings retain pending review. Do not reopen the RU choice or treat copy approval as release evidence.
